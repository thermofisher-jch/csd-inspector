...
<script type="text/javascript">
	...
	$(function(){
		...
		% if request.path == '/trace/pgm':
			% if 'show_hide_session/trace/pgm' in request.session and request.session['show_hide_session/trace/pgm']:
				var show_hide_session = ${request.session['show_hide_session/trace/pgm'] | n};
			...
			% if 'file_pending/trace/pgm' in request.session and request.session['file_pending/trace/pgm']:
				var l = Ladda.create(document.getElementById('csv_download'));
				l.start()
				check_for_updates(l, "${request.session['file_pending/trace/pgm']}")
			% endif
		% elif request.path == '/trace/proton':
			% if 'show_hide_session/trace/proton' in request.session and request.session['show_hide_session/trace/proton']:
				var show_hide_session = ${request.session['show_hide_session/trace/proton'] | n};
				show_hide_columns(show_hide_session);
			% else:
				var show_hide_session = global_columns_default;
				show_hide_columns(show_hide_session);
			...
			% if 'file_pending/trace/proton' in request.session and request.session['file_pending/trace/proton']:
				var l = Ladda.create(document.getElementById('csv_download'));
				l.start()
				check_for_updates(l, "${request.session['file_pending/trace/proton']}")
			% endif
		% endif
...
<div class="modal fade remove_confirmation" tableindex="-1" role="dialog" aria-labelledby="remove_confirmation" aria-hidden="true">
	<div class="modal-dialog modal-sm">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<form id="remove_filter_form" action="${request.route_path('analysis_delete_saved_filter')}" method="POST">
					<input type="hidden" name="csrf_token" value="${request.session.get_csrf_token()}"/>
