OT_plots<-function(work_dir,file_name,machine,target,version) {
    ...
    if( header_len ==36){
        OT2=TRUE;
        machine="OT2"
		# OT2 headers introduced in v758
        header=c("Step","Thermistor0Temperature","Thermistor1Temperature","Thermistor2Temperature","Thermistor3Temperature",
            "Thermistor4Temperature","P_SensorCur.Pressure","RawA2D","TEC0CurrentTemp","SetTemp","prev.OnTime","OnTime",
            "TecDir","AdjustedSetTemp","Heater0CurrentTemp","SetTemp.1","PWMdutyCycle","Heater1CurrentTemp","SetTemp.2",
            "PWMdutyCycle.1","MotorPowerOscillation","Raw","SolenoidPumpTotalVolume","DispensedVolume","OilPumpStatus",
        ...
        if( ! (all(names(dataSet)==header ))) {
			# OT2 headers from before v758
			header[11]="PWMdutyCycle"
			header[17]="PWMdutyCycle.1"
			header[20]="PWMdutyCycle.2"
