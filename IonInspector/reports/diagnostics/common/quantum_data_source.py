...
class QuantFeatureEvaluator:
    ...
    def find_layout_features(self):
        if self._feature_layout is None:
            ...
            null_row_indices = (
                pd.Series(range(len(practice_read)))
            ...
            if block_count == 2:
                ...
                raise ValueError(
                    "Unexpected header block count of <"
                    + str(block_count)
                    + "> at <"
            ...
            if column_count == 5:
                ...
                raise ValueError(
                    "Unexpected post-headers block column count of <"
                    + str(column_count)
                    + ">"
                )
            ...
            self._feature_layout = QuantFeatureLayout(
                ...
                barcode_offset=_barcode_offset,
                barcode_row_count=_barcode_row_count,
            )
...
class QuantDataSource:
    ...
    def quant_header_table(self):
        if self._header_table is None:
            ...
            self._header_table = header_table.pivot(
