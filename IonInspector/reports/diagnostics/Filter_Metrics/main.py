...
def create_plot(plot_data, image_path):
    ...
    ax.bar(
        ...
        align="center",
        alpha=0.5,
        color=color_scheme[: len(plot_data.keys())],
    )
    ax.set_xticks(y_pos)
...
def bar_value_to_header(value, total_values):
    ...
    return (
        str(value)
        + "\n"
        + str(int(float(value) / float(total_values) * 100.0))
        + "% of all"
    )
...
def execute(archive_path, output_path, archive_type):
    ...
    try:
        ...
        quality_filter = (
            base_caller.get("Filtering", dict())
            .get("ReadDetails", dict())
            .get("lib", dict())
        ...
        if not os.path.exists(sigproc_path):
            raise Exception(
                "Cannot find signal processing log or a thumbnail S5/Proton CSA was not uploaded."
            )
        ...
        basecaller_path = os.path.join(
            archive_path, "basecaller_results", "BaseCaller.json"
        )
        ...
        write_results_from_template(
            {
                "pwc_beads": beads,
                "pwc_beads_perc": float(beads) / float(pwc_total) * 100.0,
                "pwc_empties": empties,
                "pwc_empties_perc": float(empties) / float(pwc_total) * 100.0,
                "pwc_pinned": pinned,
                "pwc_pinned_perc": float(pinned) / float(pwc_total) * 100.0,
                "pwc_ignored": ignored,
                "pwc_ignored_perc": float(ignored) / float(pwc_total) * 100.0,
                "pwc_total": pwc_total,
                "loaded_library": library,
                "loaded_library_perc": float(library) / float(beads) * 100.0,
                "loaded_tfbead": tfbead,
                "loaded_tfbead_perc": float(tfbead) / float(beads) * 100.0,
                "loaded_duds": duds,
                "loaded_duds_perc": float(duds) / float(beads) * 100.0,
                "libarary_low_quality_high_ppf": low_quality_high_ppf,
                "libarary_low_quality_high_ppf_prec": float(low_quality_high_ppf)
                / float(library)
                * 100.0,
                "libarary_polyclonal": polyclonal,
                "libarary_polyclonal_perc": float(polyclonal) / float(library) * 100.0,
                "libarary_low_quality_bad_key": low_quality_bad_key,
                "libarary_low_quality_bad_key_perc": float(low_quality_bad_key)
                / float(library)
                * 100.0,
                "libarary_low_quality_short_read_key": low_quality_short_read,
                "libarary_low_quality_short_read_perc": float(low_quality_short_read)
                / float(library)
                * 100.0,
                "libarary_low_quality_failed_keypass": low_quality_failed_keypass,
                "libarary_low_quality_failed_keypass_perc": float(
                    low_quality_failed_keypass
                )
                / float(library)
                * 100.0,
                "libarary_primer_dimer": primer_dimer,
                "libarary_primer_dimer_perc": float(primer_dimer)
                / float(library)
                * 100.0,
                "libarary_low_quality_quality_trim": low_quality_quality_trim,
                "libarary_low_quality_quality_trim_perc": float(
                    low_quality_quality_trim
                )
                / float(library)
                * 100.0,
                "libarary_low_quality_tag_trim": low_quality_tag_trim,
                "libarary_low_quality_tag_trim_perc": float(low_quality_tag_trim)
                / float(library)
                * 100.0,
                "libarary_final_library_isps": final_library_isps,
                "libarary_final_library_isps_perc": float(final_library_isps)
                / float(library)
                * 100.0,
