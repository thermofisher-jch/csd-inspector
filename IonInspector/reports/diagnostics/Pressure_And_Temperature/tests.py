...
class PressureAndTemperatureOkayTestCase(SimpleTestCase):
    def test_get_pressure_and_temp_pgm(self):
        files = {
            "explog_final.txt": "PGMTemperature: 30.42 - 31.14\n"
        ...
        with TemporaryDirectory(files) as archive_path:
            (
                ...
                level,
                flow_data,
            ) = get_pressure_and_temp(archive_path, "PGM_Run")
    ...
    def test_get_pressure_and_temp_proton(self):
        files = {
            "explog_final.txt": "ProtonTemperature:29.408384 30.336662\n"
        ...
        with TemporaryDirectory(files) as archive_path:
            (
                ...
                level,
                flow_data,
            ) = get_pressure_and_temp(archive_path, "Proton")
            self.assertEquals(level, INFO)
    ...
    def test_get_pressure_and_temp_s5(self):
        files = {
            "explog_final.txt": "PGMPressure:0.309028 7.883653\n"
            "ExperimentInfoLog:\n"
        ...
        with TemporaryDirectory(files) as archive_path:
            (
                ...
                level,
                flow_data,
            ) = get_pressure_and_temp(archive_path, "S5")
    ...
    def test_get_pressure_and_temp_valk(self):
        files = {
            "explog_final.txt": "ExperimentInfoLog:\n"
        ...
        with TemporaryDirectory(files) as archive_path:
            (
                ...
                level,
                flow_data,
            ) = get_pressure_and_temp(archive_path, "Valkyrie")
...
class PressureAndTemperatureAlarmCase(SimpleTestCase):
    ...
    def test_get_pressure_and_temp_proton_alert_temp(self):
        ...
        with TemporaryDirectory(files) as archive_path:
            ...
            # temp
            self.assertEquals(
                temperature_message, "Temperature is cold (18.21) at flow 1"
            )
