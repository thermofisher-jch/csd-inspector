...
def provision():
    ...
    sudo(
        "mkdir -p {dir}/postgres; chown deploy:deploy {dir}; chmod 777 {dir}/postgres".format(
            dir=HOST_DATA_DIR
        )
    )
    sudo(
        "mkdir -p {dir}/celery; chown deploy:deploy {dir}; chmod 777 {dir}/celery".format(
            dir=HOST_DATA_DIR
        )
    )
    sudo(
        "mkdir -p {dir}/media; chown deploy:deploy {dir}; chmod 777 {dir}/media".format(
            dir=HOST_DATA_DIR
        )
    )
    # Setup db backups
    put(
        "./conf/backup_inspector_database.sh",
        "/etc/cron.hourly/backup_inspector_database",
        use_sudo=True,
    )
    sudo("chmod 755 /etc/cron.hourly/backup_inspector_database")
    put(
        "./conf/backup_inspector_logs.sh",
        "/etc/cron.hourly/backup_inspector_logs",
        use_sudo=True,
    )
...
def deploy(tag=None, force=None):
    ...
    if any([host in env.host for host in PRODUCTION_HOSTS]):
        if (
            raw_input(
                "This looks like a deploy to production! Enter PRODUCTION to continue: "
            )
            != "PRODUCTION"
        ):
            exit()
    with cd(HOST_DATA_DIR + "/inspector"):
        ...
        upload_template(
            "./conf/nginx.conf",
            "/etc/nginx/sites-enabled/inspector.conf",
            {},
            use_sudo=True,
            backup=False,
        )
